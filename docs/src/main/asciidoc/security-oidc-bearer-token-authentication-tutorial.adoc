...
You can either create a new Maven project with the `oidc` extension or add the extension to an existing Maven project.
...
Then, in an OpenID Connect `Dev UI` card, click the `Keycloak provider` link .
 ...
 * Log out and log back in as `admin` (password: `admin`), with both `admin` and `user` roles.
