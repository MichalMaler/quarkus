...
In this case, the authorization code flow is triggered only when the HTTP `Authorization` request header is not set with the `Bearer` scheme and a bearer access token.
...
This URL logs the user out of all applications they are currently logged into.
...
[IMPORTANT]
====
To prevent the user from being redirected to this page for re-authentication, ensure that this error endpoint is publicly accessible.
====
...
When the user is redirected back to Quarkus to complete the authorization code flow, Quarkus expects the request URI to include the `state` query parameter and that it matches the current state cookie value.
...
This is ideal for those seeking customized solutions for token state management, especially when standard server-side storage does not meet your specific requirements.
...
An access token is only required if the endpoint needs to perform the following:
...
OIDC providers use the link:https://openid.net/specs/openid-connect-backchannel-1_0.html[Back-channel logout] to log out the current user from all the applications they are currently logged into, bypassing the user agent.
...
If the session at the OIDC provider is still active, users are automatically re-authenticated without having to re-enter their credentials.
...
The OIDC provider might challenge the user again if the user's session with this provider is still active, which can happen if the session is configured to last longer than the ID token.
...
You can further optimize this process by adding a simple JavaScript function that pings your Quarkus endpoint periodically to simulate user activity, thereby minimizing the time during which the user might need to be re-authenticated.
...
The returning user with an expired ID token will have to re-authenticate at the OIDC provider endpoint once the refresh token expires.
====
...
. Configure `Dev services for Keycloak` to import a custom realm file into Keycloak before running the tests:
+
[source,properties]
----
%prod.quarkus.oidc.auth-server-url=http://localhost:8180/realms/quarkus
